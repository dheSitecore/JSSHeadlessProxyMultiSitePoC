 ...
 */
const bundlePath = process.env.SITECORE_JSS_SERVER_BUNDLE || `./dist/excercises/server.bundle`;
...
const config = {
   ...
   */
  apiHost: process.env.SITECORE_API_HOST || 'http://jss-experiments-dp.dev.local',
  /**
   ...
   * Required.
   */
  apiKey: process.env.SITECORE_API_KEY || serverBundle.apiKey || '{43E5BAA2-FB9E-4621-BE34-B06DCE2AB859}',
  /**
  ...
  createViewBag: (request, response, proxyResponse, layoutServiceData) => {
    ...
    return fetch(
      `${config.apiHost}/sitecore/api/jss/dictionary/excercises/${language}?sc_apikey=${
        config.apiKey
      }`
    )
